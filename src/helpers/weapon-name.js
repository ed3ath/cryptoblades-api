...
// basic weapon (TYPE)
const basic = (rng) => getRandom(rng, names.type);
...
const material = (rng) => {
  const prefix = getRandom(rng, names.prefix);
  const type = getRandom(rng, names.type);
  const materialT = getRandom(rng, names.material);

  return `${prefix} ${materialT} ${type}`;
};

// special weapon (SPC)
const special = (rng) => getRandom(rng, names.specialtype);
