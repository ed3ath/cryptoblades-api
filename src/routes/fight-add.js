...
exports.route = (app) => {
  app.get('/leaderboard/fight/add', async (req, res) => {
    ...
    const { hash, accountAddress, characterId, characterLevel, weaponId, enemyId, wonFight, enemyRoll, playerRoll, xpGain, skillGain, gas } = req.query;
    ...
    try {
      await DB.$fights.replaceOne({ hash }, { hash, accountAddress, characterId, characterLevel, weaponId, enemyId, wonFight, enemyRoll, playerRoll, xpGain, skillGain, gas }, { upsert: true });
    } catch(error) {
